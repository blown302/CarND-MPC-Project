...
unsigned int getNumIterationsForLatency() {
    auto current_time = getCurrentTime();
    double latency_in_seconds = .1;
//    double latency_in_seconds = (current_time - prev_time) / 1000. - dt;
    lat = latency_in_seconds;
    prev_time = current_time;
    cout << "latency in seconds: " << latency_in_seconds << endl;
    if (latency_in_seconds > N * dt || latency_in_seconds < 0) return 0;
    return static_cast<unsigned int>(ceil(latency_in_seconds / dt));
}
...
//    cout <<  "delta time bewteen calls: " << (current_time - prev_time) / 1000.0 << endl;
//    cout << "delta time between operations: " << dt << endl;
    AD<double> cost = 0;

    cout << "error weight: " << error_weight << endl;
    cout << "psi error weight: " << psi_error_weight << endl;

    // calculate cost
    for (auto i = 0; i < N; i++) {
      cost += error_weight * CppAD::pow(vars[cte_start + i], 2);
      cost += psi_error_weight * CppAD::pow(vars[epsi_start + i], 2);
      cost += CppAD::pow(vars[v_start + i] - calc_v, 2);
...
//    for (auto i = 0; i < fg.size(); i++) {
//        cout <<  "fg print with index: " << i << ": " << fg[i] << endl;
//    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, double throttle, double steering_angle) {
  ...
  x += v * lat * cos(psi);
  y += v * lat * sin(psi);
  psi += prev_steering_ * v * lat / Lf;
  v += prev_throttle_ * lat;
  cte = y - cte;
  epsi = psi - epsi;
...
//    vars_upperbound[i] = numeric_limits<double>::max();
  ...
  auto num_iterations_latency = getNumIterationsForLatency();
//  cout << "freezing actuators for " << num_iterations_latency << " iterations" << endl;

//  for (auto i = 0; i < num_iterations_latency; i++) {
//
//
//      vars_upperbound[delta_start + i] = prev_steering_;
//      vars_lowerbound[delta_start + i] = prev_steering_;
//      vars_upperbound[a_start + i] = prev_throttle_;
//      vars_lowerbound[a_start + i] = prev_throttle_;
//  }
